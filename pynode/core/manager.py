...
# ----------------------------------
    ...
    # account address of worker node owner
    eth_worker_node_account = None
    # base global eth_connector settings for fast access from any module
    eth_use = None
    eth_host = None
    eth_abi_path = None
    eth_pandora = None
    ...
    # compilation mode
    # 0 = PROD mode (use production integration layers)
    # 1 = DEV  mode (use dummy integration layers realization)
    run_mode = 1
...
# ----------------------------------
    def on_property_value_change(self):
        if self.web_socket_enable == 'True':
            # if false we do not need to even import WebSocket class
